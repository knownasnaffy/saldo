...
class DatabaseManager:
    ...
    def _get_connection(self) -> sqlite3.Connection:
        ...
        if self._connection is None:
            try:
                ...
                raise DatabaseError(
                    f"Database file access error: {e}",
    ...
    def initialize_database(self) -> None:
        ...
        try:
            ...
            if "database is locked" in str(e).lower():
                raise DatabaseError(
                    f"Database is locked by another process: {e}",
    ...
    def save_transaction(self, transaction: Dict[str, Any]) -> int:
        ...
        try:
            ...
            if "items" in str(e):
                raise DatabaseError(
                    "Items must be non-negative", f"Database constraint violation: {e}"
                )
            elif "cost" in str(e):
                raise DatabaseError(
                    "Cost must be non-negative", f"Database constraint violation: {e}"
                )
            else:
                raise DatabaseError(f"Data integrity error: {e}")
        except sqlite3.Error as e:
        ...
        try:
            ...
            if row:
                ...
                if not isinstance(balance, (int, float)):
                    raise DatabaseError(
                        "Invalid balance data in database",
            ...
            if config:
                ...
                if not isinstance(balance, (int, float)):
                    raise DatabaseError(
                        "Invalid initial balance in configuration",
                        f"Balance value: {balance} (type: {type(balance)})",
                    )
                return float(balance)
