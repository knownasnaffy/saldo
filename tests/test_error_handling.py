...
class TestInputValidation:
    ...
    def test_setup_account_extreme_values(self):
        ...
        with pytest.raises(
            ValidationError, match="Initial balance seems unusually large"
        ):
    ...
    def test_calculate_cost_extreme_values(self):
        ...
        with pytest.raises(
            ValidationError, match="Number of items seems unusually large"
        ):
    ...
    def test_add_transaction_extreme_values(self):
        ...
        with pytest.raises(
            ValidationError, match="Payment amount seems unusually large"
        ):
        ...
        with pytest.raises(
            ...
            self.transaction_manager.add_transaction(
                500000, 0.0
            )  # This will fail on item count first
...
class TestDatabaseErrorHandling:
    ...
    def test_database_locked_error(self):
        ...
        try:
            ...
            with patch.object(db_manager, "_get_connection") as mock_get_conn:
                ...
                with pytest.raises(
                    DatabaseError, match="Database is locked by another process"
                ):
    ...
    def test_disk_space_error(self):
        ...
        try:
            ...
            with patch.object(db_manager, "_get_connection") as mock_get_conn:
                ...
                with pytest.raises(
                    DatabaseError, match="Failed to initialize database"
                ):
                    db_manager.initialize_database()
        finally:
            if os.path.exists(temp_db.name):
    ...
    def test_corrupted_configuration_data(self):
        ...
        try:
            ...
            cursor.execute(
                "INSERT INTO configuration (rate_per_item, initial_balance) VALUES (?, ?)",
                (-1.0, 0.0),
            ...
            with pytest.raises(
                ConfigurationError, match="Invalid rate in configuration"
            ):
...
class TestCLIErrorHandling:
    ...
    def test_add_transaction_large_items_confirmation(self, mock_tm_class):
        ...
        result = self.runner.invoke(
            cli, ["add-transaction"], input="1500\nn\n5\n12.50\n"
        )
...
class TestErrorRecovery:
    ...
    def test_balance_command_transaction_history_error(self, mock_tm_class):
        ...
        mock_tm.db_manager.get_transactions.side_effect = DatabaseError(
    ...
    def test_configuration_validation_error_recovery(self, mock_tm_class):
        ...
        mock_tm.setup_account.side_effect = ValidationError(
            "Rate per item seems unusually high", "Received value: 1500.0"
        )

        result = self.runner.invoke(
            cli, ["setup", "--rate", "1500.0", "--balance", "0"]
        )
