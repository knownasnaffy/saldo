...
class TestEndToEndWorkflows:
    ...
    def test_complete_workflow_setup_to_balance(self):
        ...
        setup_result = self.runner.invoke(cli, [
            'setup', '--rate', '2.50', '--balance', '10.00'
        ])
        ...
        transaction1_result = self.runner.invoke(cli, [
            'add-transaction', '--items', '3', '--payment', '5.00'
        ])
        ...
        transaction2_result = self.runner.invoke(cli, [
            'add-transaction', '--items', '2', '--payment', '20.00'
        ])
    ...
    def test_workflow_with_zero_balance_result(self):
        ...
        transaction_result = self.runner.invoke(cli, [
    ...
    def test_workflow_with_detailed_balance_history(self):
        ...
        self.runner.invoke(cli, ['add-transaction', '--items', '3', '--payment', '4.00'])
        self.runner.invoke(cli, ['add-transaction', '--items', '1', '--payment', '3.00'])
        self.runner.invoke(cli, ['add-transaction', '--items', '2', '--payment', '1.00'])
    ...
    def test_workflow_with_zero_items_transaction(self):
        ...
        transaction_result = self.runner.invoke(cli, [
            'add-transaction', '--items', '0', '--payment', '5.00'
        ])
        assert transaction_result.exit_code == 0
...
class TestDataPersistence:
    ...
    def test_transaction_history_persistence(self):
        ...
        self.runner.invoke(cli, ['setup', '--rate', '2.00', '--balance', '0.00'])
        self.runner.invoke(cli, ['add-transaction', '--items', '5', '--payment', '8.00'])
        self.runner.invoke(cli, ['add-transaction', '--items', '2', '--payment', '6.00'])
...
class TestErrorRecoveryAndEdgeCases:
    ...
    def test_commands_without_setup(self):
        ...
        transaction_result = self.runner.invoke(cli, [
            'add-transaction', '--items', '5', '--payment', '10.00'
        ])
        assert transaction_result.exit_code != 0
    ...
    def test_invalid_input_recovery_setup(self):
        ...
        setup_result = self.runner.invoke(cli, ['setup'], input='abc\n-2.0\n0\n2.50\nxyz\n10.00\n')
        assert setup_result.exit_code == 0
    ...
    def test_invalid_input_recovery_transaction(self):
        ...
        transaction_result = self.runner.invoke(cli, ['add-transaction'], 
                                              input='abc\n-1\n3\nxyz\n6.00\n')
        assert transaction_result.exit_code == 0
    ...
    def test_large_numbers_handling(self):
        ...
        large_items_result = self.runner.invoke(cli, ['add-transaction'], 
                                              input='2000\ny\n1000.00\n')
    ...
    def test_edge_case_zero_rate_validation(self):
        ...
        zero_rate_result = self.runner.invoke(cli, ['setup', '--rate', '0', '--balance', '10.00'])
    ...
    def test_edge_case_negative_items_validation(self):
        ...
        # Try negative items
        negative_items_result = self.runner.invoke(cli, [
            'add-transaction', '--items', '-5', '--payment', '10.00'
        ])
    ...
    def test_database_directory_creation(self):
        ...
        try:
            ...
            setup_result = self.runner.invoke(cli, ['setup', '--rate', '2.00', '--balance', '5.00'])
            assert setup_result.exit_code == 0
            assert "Account setup completed successfully!" in setup_result.output
...
class TestCLIHelpAndVersion:
    ...
    def test_main_cli_help(self):
        ...
        assert "Saldo - A command-line balance tracking application" in help_result.output
